...
email VARCHAR(45) ,
address NVARCHAR(45) ,
FOREIGN KEY (typecustomer_id) REFERENCES typecustomers(typecustomer_id) ON DELETE CASCADE
);
...
FOREIGN KEY (typeservice_id) REFERENCES typeservices(typeservice_id) ON DELETE CASCADE
);
...
FOREIGN KEY (bonusservice_id) REFERENCES bonusservices(bonusservice_id) ON DELETE CASCADE
) ;
